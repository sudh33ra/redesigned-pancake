...
$sp_id = isset($_POST['load_id']) ? sanitize_text_field($_POST['load_id']) : 0;
$nonce = isset($_POST['nonce']) ? sanitize_text_field($_POST['nonce']) : 0;

/**
 * Check nonce
 */
if( !wp_verify_nonce($nonce, $sp_id) ){
    die(__('Cheating huh?', 'wp-support-plus-responsive-ticket-system'));
}

global $wpsupportplus, $wpdb;
$menu = $wpdb->get_row("select * from {$wpdb->prefix}wpsp_support_menu where id=".$sp_id);
...
<form action="" onsubmit="return validate_wpsp_admin_popup_form(this);">
    ...
    <?php do_action('wpsp_get_delete_support_page_menu', $menu);?>
    
    <input type="hidden" name="action" value="wpsp_set_delete_support_menu" />
    <input type="hidden" name="load_id" value="<?php echo htmlentities($sp_id)?>" />
    <input type="hidden" name="nonce" value="<?php echo wp_create_nonce($sp_id)?>" />
...
function validate_wpsp_admin_popup_form(obj){
    ...
    <?php do_action('validate_wpsp_get_delete_support_menu');?>
